		...
		bezier: function (options) {
			...
			me.on = function (el1, el2) {
				...
				if ($el1.length && $el2.length) {
					...
					var new_d = $('.new_dashboard').offset().left;

					//var el1pos = $(el1).offset('.new_dashboard');
					//$('.new_dashboard').html(Math.round(el1pos.top) + ', ' + Math.round(el1pos.left));
					...
					if ((el2pos.top + (el2H / 2)) <= (el1pos.top + (el1H / 2))) {
						...
						svgtop = Math.round(el2pos.top + el2H / 2) - (settings.strokeWidth + $('.new_dashboard').offset().top);
						...
						svgtop = Math.round(el1pos.top + el1H / 2) - (settings.strokeWidth + $('.new_dashboard').offset().top);
						cpt = Math.round(svgwidth * Math.min(svgheight / 300, 1));
						if (cpt < 0) {
							svgwidth = 4;
							p = "M0," + settings.strokeWidth + " V" + cpt + ",0 " + (svgwidth - cpt) + "," + (svgheight + settings.strokeWidth) + " " + svgwidth + "," + (svgheight + settings.strokeWidth);
							settings.strokeWidth = 4;
							if (el1pos.left < el2pos.left) {
								svgleft = svgleft - el2W * 2;
							}
							else {
								svgleft = svgleft - el2W / 2;
							}
							//svgwidth = '4px !important';
					...
					$ropebag = $('#ropebag').length ? $('#ropebag') : $('.new_dashboard').append($("<div id='ropebag' />")).find('#ropebag');
